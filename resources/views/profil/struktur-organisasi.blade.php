...
<h1 class="py-4 text-4xl font-bold tracking-tight text-gray-900 dark:text-white">Struktur Organisasi</h1>
...
<hr class="mb-3"/>
<img src="{{asset('storage/background/struktur-organisasi-pdam-pbg.png')}}"/>
...
<th scope="col" class="px-6 py-3">Jabatan</th>
        ...
        @foreach($employees->where('position_id', $position->id) as $employee)
    ...
    <!-- @foreach ($positions as $position)
            <div class="block card cabang pdam w-full p-5 text-white my-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mx-10">
                            ...
                            @foreach ($posts as $item)
                                <a href="/post/{{$item->id}}">
                                    <div class="flex flex-row gap-x-5 mb-16">
                                        <div class="w-1/2">
                                            <img class="h-28 w-full object-cover rounded-md"
                                                src="{{asset('storage/' . $item->image)}}" alt="Post image">
                                        </div>
                                        <div class="w-1/2 flex flex-col">
                                            <div class="short-title">{{ $item->shortTitle() }}</div>
                                            <div class="date mt-2">
                                                {{Carbon\Carbon::parse($item->published_at)->isoFormat('D MMMM Y')}}</div>
                                        </div>
                                    </div>
                                </a>
                            @endforeach
                ...
                <div class="col-span-2 ml-4">
                    <div class="flex flex-col mb-8">
                        ...
                        <img class="my-4" src={{ asset('storage/background/struktur-organisasi-pdam-pbg.png') }}>
                        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 mt-7">
                            <thead
                                class="text-xs text-gray-700 uppercase bg-gray-100 dark:bg-gray-700 dark:text-gray-400">
                                <tr>
                                    <th scope="col" class="text-center px-4 py-3">No</th>
                                    <th scope="col" class="px-6 py-3">Jabatan</th>
                                    <th scope="col" class="px-6 py-3">
                                        Nama
                                    </th>
