<x-layout>
  <!-- <div class="row list">
    <div class="column middle px-5">
    <div class="font-poppins w-full pt-5 pb-5">
        <div class="col justify-between p-1 leading-normal">
            <h1 class="py-4 text-4xl font-bold tracking-tight text-gray-900 dark:text-white">Profil Cabang dan Unit IKK</h1>
            <p class="py-4 mb-3 text-md text-gray-700 dark:text-gray-400">Cabang dan Unit IKK adalah pelayanan yang dikelola PDAM Kabupaten Purbalingga pada tiap Ibu Kota Kecamatan di wilayah pelayanan.</p>
            <hr/>
            <h1 class="py-4 text-4xl font-bold tracking-tight text-gray-900 dark:text-white">CABANG</h1>
            @php
            $no=1;
            @endphp
            @foreach ($cabang as $item)
            @if($item->office->isNotEmpty())
            <div class="block card cabang pdam w-full p-5 text-white mb-4">
              <ul>
                <li class="uppercase mb-2"><b>{{$no++}}. CABANG {{$item->name}}</b></li>
                <li class="mb-3 ml-4">Kepala Cabang : {{$item->branch_manager}}</li>
                @if($item->office->isNotEmpty())
                @foreach($item->office as $office)
                <li class="ml-4 mb-2">Kantor Cabang : {{$office->sub_district}}</li>
                <li class="ml-4 mb-2">Alamat Kantor : {{$office->address}}</li>
                <li class="ml-4 mb-2">Maps : <a href="{{$office->maps}}">{{$office->maps}}</a></li>
                <li class="mb-3 ml-4">Nomor Layanan : {{$office->service_number}}</li>
                @endforeach
                @endif
              </ul>
            </div>
    ...
    <div class="column side px-5 pt-7">
   @include('components.list-post', ['posts' => $posts])
   </div>
   </div> -->
    ...
    <div>
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mx-10">
        ...
        <div class="col-span-2 ml-4">
          <div class="flex flex-col">
            <div class="title-visi mb-4">Cabang & Unit IKK</div>
            ...
            <p class="py-4 text-md text-gray-700 dark:text-gray-400">Cabang dan Unit IKK adalah pelayanan yang dikelola
              PDAM Kabupaten Purbalingga pada tiap Ibu Kota Kecamatan di wilayah pelayanan.</p>
            <h1 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white mb-5">CABANG</h1>
            @php
        $no = 1;
      @endphp
            @foreach ($cabang as $item)
