        ...
        <div class="flex flex-col">
          <div>
            <ul>
              ...
              {{ request()->is('berita') ? 'font-semibold text-blue-600' : '' }}"> <!-- cek aktif atau tidak -->
                <svg width="8" height="14" viewBox="0 0 8 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path fill-rule="evenodd" clip-rule="evenodd" d="M7.157 7.71114L1.5 13.3681L0.0859985 11.9541L5.036 7.00414L0.0859985 2.05414L1.5 0.640137L7.157 6.29714C7.34447 6.48466 7.44978 6.73897 7.44978 7.00414C7.44978 7.2693 7.34447 7.52361 7.157 7.71114Z" fill="#6E6A6A"/>
                </svg>
          ...
          @foreach ($artikelLain as $item)
              ...
              <a href="/post/{{$item->id}}">
                  <div class="flex flex-row gap-x-4 mb-4">
                      <div class="w-1/2">
                          <img class="h-24 w-full object-cover rounded-md" src="{{asset('storage/'.$item->image)}}" alt="Post image">
                      </div>
                      <div class="w-1/2 flex flex-col">
                        <div class="short-title">{{ $item->shortTitle() }}</div>
                        <div class="date mt-3">{{\Carbon\Carbon::parse($item->published_at)->format('d j Y')}}</div>
                      </div>
                  </div>
              </a>
          @endforeach
